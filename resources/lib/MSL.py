...
class MSL(object):
    ...
    def __init__(self, kodi_helper):
      ...
      #TODO instanciate crypto
    
      if self.file_exists(self.kodi_helper.msl_data_path, 'msl_data.json'):
          self.init_msl_data()

      if crypto.handshake_required() && self.kodi_helper.get_esn():
    ...
    def __decrypt_payload_chunks(self, payloadchunks):
        ...
        for chunk in payloadchunks:
            ...
            # Decrypt the text
            plaintext = crypto.decrypt(self.keyId, base64.standard_b64decode(encryption_envelope['iv']), 
    ...
    def __encrypt(self, plaintext):
        return json.dumps(self.crypto.encrypt(plaintext))
    ...
    def __perform_key_handshake(self):
        ...
        if resp and resp.status_code == 200:
            ...
            crypto.parse_handshake_result(headerdata)
        else:
            self.kodi_helper.log(msg='Key Exchange failed')
